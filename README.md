...
2. Write 3 specs for the same problem in `specs/`:
...
3. Run via Hermes, save SQL to `outputs/<round>_<model>_<spec>.sql`:
   - Round 1: Sonnet × {A, B, C}
   - Round 2: Opus × {A, B, C}
   - Round 3: Spec B × {Sonnet, Opus, Haiku}
4. Score each output with the rubric below, log to wan.db, write findings to `analysis/`.
...
```
specs/      # spec_a.md, spec_b.md, spec_c.md
outputs/    # <round>_<model>_<spec>.sql
analysis/   # metrics.json, comparison.md, report.md
```
